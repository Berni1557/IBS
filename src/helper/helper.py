...
def str2bool(v: Union[str, bool]) -> bool:
    """
    Parse a boolean flag from argparse string inputs.

    Accepts common truthy/falsey strings (case-insensitive):
      True:  "yes", "true", "t", "y", "1", "on"
      False: "no",  "false","f","n","0","off"

    If a bool is passed (e.g., programmatic use), it is returned unchanged.

    Raises:
        argparse.ArgumentTypeError: if `v` cannot be interpreted as a boolean.
    ...
    True
    """
    # If already a boolean (e.g., internal call), pass through.
    if isinstance(v, bool):
        return v

    if not isinstance(v, str):
        raise argparse.ArgumentTypeError("Boolean value expected (got non-string).")

    # Normalize: strip spaces and make case-insensitive.
    val = v.strip().casefold()

    truthy = {"yes", "true", "t", "y", "1", "on"}
    falsy  = {"no", "false", "f", "n", "0", "off"}
